...
public class MPayOcsClient extends DiameterRoClient implements Future <RoCCAnswer> {
  ...
  public final void sendEvent(RoCCRequest eventAuthRequest) {
    try {
  ...
  private Future<RoCCAnswer> sendEventAuth(RoCCRequest eventAuthRequest ) {
    try {
      log.debug("Received: " + eventAuthRequest);
      ClientRoSession clientRoSession = getSession();
      log.debug("clientRoSessionId: " + clientRoSession.getSessionId());
  ...
  public final RoCCAnswer sendEventAuthAndBlock(RoCCRequest eventAuthRequest) {
    try {
      //phaser.register();
      //countDownLatch = new CountDownLatch(1);
      //countDownLatch = eventAuthRequest.getCountDownLatch();
      Future<RoCCAnswer> er = sendEventAuth(eventAuthRequest);
  ...
  protected RoCreditControlRequest createCCR(int ccRequestType, int requestNumber, ClientRoSession ccaSession, RoCCRequest eventAuthRequest)
      throws Exception {
  ...
  private RoCCAnswer getEventAuthorizationResponse(RoCCRequest eventAuthRequest, RoCreditControlAnswer answer) {
    RoCCAnswer eventAuthResponse = new RoCCAnswer();
  ...
  public void doCreditControlAnswer(ClientRoSession session, RoCreditControlRequest request, RoCreditControlAnswer answer)
    ...
    synchronized (this) {
      try {
        ...
        RoCCRequest eventAuthRequest = inFlightTxns.get(sessionId);
        if (eventAuthRequest != null) {
          // Build an EventAuth Response
          RoCCAnswer eventAuthResponse = getEventAuthorizationResponse(eventAuthRequest, answer);
