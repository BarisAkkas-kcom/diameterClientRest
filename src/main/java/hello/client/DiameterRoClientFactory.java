...
public class DiameterRoClientFactory implements /*ClientRoSessionListener,*/ NetworkReqListener,
    EventListener<Request, Answer>/*,Future<RoCca>*/ {
  ...
  static final String clientConfig = "client-jdiameter-config.xml";
  ...
  //private static Map<String, CountDownLatch> inFlightSessions = new HashMap<String, CountDownLatch>();
  private static StackImpl stack;
  private static ISessionFactory sessionFactory;
  private static int ccRequestNumber;

  private static ApplicationId applicationID = ApplicationId.createByAuthAppId(0, 4);
...
//          applicationID,ClientRoSession.class, (Object) null);
      ...
      log.debug("No of inflight requests after adding txin = " + roCcr.getTransactionId() + " session id = " + clientRoSession.getSessionId() +
          "=" + inFlightTxns.size());
  ...
  private Request createCCR(int ccRequestType, int requestNumber, Session ccaSession, RoCcr roCcr)
      throws Exception {
...
//        .createRequest(RoCreditControlRequest.code, applicationID, realmName));
    ...
    String serviceContextId = "32270@3gpp.org";
...
//    }
    ...
    String subscriptionIdData = roCcr.getMsisdn();
  ...
  private RoCca getEventAuthorizationResponse(RoCcr roCcr, Answer answer) {
    ...
    try {
      final long resultCode = answer.getResultCode().getUnsigned32();
      roCca.setReturnCode(String.valueOf(resultCode));
      roCca.setSuccess(resultCode == 2001);
      roCca.setTxnId(roCcr.getTransactionId());
      if (resultCode == 2001) {
        ...
        roCca.setReservedUnits(serviceSpecificUnitsString);
      }
  ...
  public RoCca get() throws InterruptedException, ExecutionException {
    ...
    log.debug("Baris returning result : " + result);
    return this.result;
  }
  ...
  public Answer processRequest(Request request) {
    log.info("Answer processRequest");
    return null;
  }
  ...
  public void receivedSuccessMessage(Request request, Answer answer) {
    ...
    try {
      ...
      log.debug(inFlightTxns.size());
      for (String key : inFlightTxns.keySet()){
        log.debug("key : " + key);
      }
      ...
      if (roCcr != null) {
        ...
        this.result = roCca;
        //inFlightSessions.get(sessionId).countDown();
        //countDownLatch = roCcr.getCountDownLatch();
        Thread.sleep(20);
        this.countDownLatch.countDown();
